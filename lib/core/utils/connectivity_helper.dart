...
class ConnectivityHelper {
  ...
  ConnectivityHelper._internal(this._connectivity) {
    ...
    _connectivity.onConnectivityChanged.listen((dynamic result) {
      if (result is List) {
        // For newer versions of connectivity_plus, which might return a list
        final resultList = result.cast<ConnectivityResult>();
        final hasConnection = resultList.any((status) => status != ConnectivityResult.none);
        _updateConnectionStatus(hasConnection ? ConnectivityResult.wifi : ConnectivityResult.none);
      } else if (result is ConnectivityResult) {
        // For older versions that return a single result
        _updateConnectionStatus(result);
      } else {
        _log.warning('Unknown connectivity result type: ${result.runtimeType}');
        // Default to checking connectivity directly
        checkConnectivity();
      }
    });
  }
  ...
  Future<bool> checkConnectivity() async {
    try {
      ...
      if (result is List) {
        // Handle list of results (newer connectivity_plus versions)
        final resultList = result.cast<ConnectivityResult>();
        final hasConnection = resultList.any((status) => status != ConnectivityResult.none);
        _updateConnectionStatus(hasConnection ? ConnectivityResult.wifi : ConnectivityResult.none);
      } else if (result is ConnectivityResult) {
        // Handle single result (older versions)
        _updateConnectionStatus(result);
      } else {
        _log.warning('Unknown connectivity result type: ${result.runtimeType}');
        // Default to disconnected if unknown type

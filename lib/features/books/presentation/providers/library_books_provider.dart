...
import 'package:modudi/features/books/data/models/book_models.dart'; // Use the new Book model
...
class LibraryBooksNotifier extends StateNotifier<LibraryState> {
  ...
  Future<void> loadBooks({String? languageCode, String? category}) async {
    try {
      ...
      // Try to apply filters, but handle index errors gracefully
      try {
        ...
        // Handle Firestore index errors by falling back to loading all books
        if (e.toString().contains('index')) {
          _logger.warning('Firestore index error, falling back to loading all books: $e');
          
          // Load all books without complex filtering
          final snapshot = await _firestore.collection('books').limit(booksPerPage).get();
          ...
          // Load all books and filter in memory
          final allBooks = snapshot.docs.map((doc) {
            return Book.fromMap(doc.id, doc.data());
          }).toList();
          
          // Filter in memory based on category and language
          final filteredBooks = allBooks.where((book) {
            bool matchesCategory = category == null || category.toLowerCase() == 'all' || 
                (book.tags?.any((tag) => tag.toLowerCase() == category.toLowerCase()) ?? false);
                
            bool matchesLanguage = languageCode == null || languageCode.toLowerCase() == 'all' || 
                book.defaultLanguage?.toLowerCase() == languageCode.toLowerCase();
          ...
          _logger.info('Loaded ${filteredBooks.length} books after in-memory filtering.');
          state = LibraryStateData(books: filteredBooks, lastUpdated: DateTime.now(), lastDocument: lastDoc);
          return;
        } else {

...
class Heading {
  ...
  Map<String, dynamic> toMap() {
    final map = <String, dynamic>{
      'firestoreDocId': firestoreDocId, // Ensure firestoreDocId is serialized
      'id': id,
  ...
  factory Book.fromMap(String docId, Map<String, dynamic> map) {
    ...
    return Book(
      ...
      volumes: map['volumes'] != null 
          ? (map['volumes'] as List)
              .map((vMap) => Volume.fromMap(vMap as Map<String, dynamic>))
              .toList()
          : null,
      headings: map['headings'] != null && (map['volumes'] == null || (map['volumes'] as List).isEmpty)
          ? (map['headings'] as List)
              .map((hMap) => Heading.fromMap(hMap as Map<String, dynamic>))
              .toList()
          : null,
    );
  }
...
class Volume {
  ...
  factory Volume.fromMap(Map<String, dynamic> map) { // Signature changed
    return Volume(
      firestoreDocId: map['firestoreDocId'] as String, // Get from map
      id: map['id'] as int?,
      title: map['title'] as String?,
      bookId: map['book_id'] as int?,
      sequence: map['sequence'] as int?,
      ...
      additionalFields: Map<String, dynamic>.fromEntries(
        map.entries.where((entry) => ![
          'firestoreDocId', 'id', 'title', 'book_id', 'sequence', 'description',
          'status', 'created_at', 'updated_at', 'chapters' // Exclude new fields from additionalFields
        ].contains(entry.key)),
      ),
      // Deserialize chapters
      chapters: (map['chapters'] as List<dynamic>?)
          ?.map((cMap) => Chapter.fromMap(cMap as Map<String, dynamic>))
          .toList(),
    );
  }
  ...
  Map<String, dynamic> toMap() {
    final map = <String, dynamic>{
      'firestoreDocId': firestoreDocId, // Added firestoreDocId
      'id': id,
      'title': title,
      'book_id': bookId,
      'sequence': sequence,
    ...
    // Serialize chapters
    if (chapters != null) { // Check if chapters list is not null
      map['chapters'] = chapters!.map((c) => c.toMap()).toList(); // Use ! because of the null check
    }
    map.addAll(additionalFields);
    map.removeWhere((key, value) => value == null); // removeWhere should be one of the last operations
    return map;
  }
...
class Chapter {
  ...
  factory Chapter.fromMap(Map<String, dynamic> map) {
    return Chapter(
      ...
      headings: (map['headings'] as List<dynamic>?)
          ?.map((hMap) => Heading.fromMap(hMap as Map<String, dynamic>))
          .toList(),
    );
  }

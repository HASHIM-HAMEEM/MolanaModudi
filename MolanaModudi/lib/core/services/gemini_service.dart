...
class GeminiService {
  ...
  Future<List<Map<String, dynamic>>> extractChapters(String text, {int maxLength = 8000, String? bookType, String? title, bool isTableOfContents = false}) async {
    try {
      ...
      // Enhanced prompt for better chapter extraction
      String prompt = '''
      Analyze the following text and extract chapter/section information in JSON format.
      
      Text to analyze:
      $text
      
      Please return a JSON array where each object represents a chapter/section with:
      - "title": The chapter/section title
      - "content": The main content of that section
      - "page_start": Estimated starting position (use 0 if unknown)
      - "page_end": Estimated ending position (use 0 if unknown)
      
      Focus on identifying clear chapter breaks, headings, and logical content divisions.
      ''';
      
      if (isTableOfContents) {
        prompt += 'This is a Table of Contents page from a book${title != null ? ' titled "$title"' : ''}. Extract the chapters and their page numbers from this content.';
      }
